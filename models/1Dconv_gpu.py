...
with torch.cuda.device(0):
    ...
    for epoch in range(num_epochs):
        X,y = gen_Xy([20000,channels],temp_dep=20)
        X,y = window(X,y,window_size=window_size)
        X = np.swapaxes(X,1,2)
        traces = Variable(torch.from_numpy(X).float()).cuda()
        result = Variable(torch.from_numpy(y).float()).cuda()
    ...
    #         labels = Variable(labels)
        ...
        optimizer.zero_grad()
        outputs = cnn(traces).cuda()
        loss = criterion(outputs, result).cuda()
